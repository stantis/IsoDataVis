...
## Labels
`ggrepel` let's you make sure your text labels don't overlap. My example is a little harsh, as I could have used `hjust = 0, nudge_x = 0.05` to move the text off of the points, but `geom_text_repel` is quick and easy. 

```{r Text Overlap, fig.show = "hold", out.width = "50%", echo = F}
...
```{r Size Queen, fig.show = "hold", out.width = "50%", include = T, echo = F}
ggplot() +
  geom_point(data = ThesisData,  aes(x = C_coll, y = N)) +
  theme_classic() +
  theme(plot.title = element_text(size = 16)) + 
...
```{r Viridis Showoff, fig.show = "hold", out.width = "30%", echo = F}
...
```{r Color, fig.show = "hold", out.width = "50%", include = T, echo = F}
ggplot() +
